...
import migrateD1All, { executeSqlFile, getSqlFiles } from './executor';
...
describe('migrateD1All', () => {
  let context: ExecutorContext;
  let options: MigrateD1AllExecutorSchema;
  ...
  it('should execute SQL migration with selected file', async () => {
    ...
    const result = await migrateD1All(options, context);
    ...
    expect(execSync).toHaveBeenNthCalledWith(1, `npx wrangler d1 execute final-database --local --file=/mock/test-project/sqlmigration1.sql --config=/mock/test-project/wrangler.toml --yes`);
    expect(execSync).toHaveBeenNthCalledWith(2, `npx wrangler d1 execute final-database --local --file=/mock/test-project/sqlmigration2.sql --config=/mock/test-project/wrangler.toml --yes`);
...
describe('executeSqlFile', () => {
  ...
  it('should execute SQL file successfully', () => {
    mockExecSync.mockImplementation(() => {});

    executeSqlFile(databaseName, projectPath, directory, sqlFile);

    expect(mockExecSync).toHaveBeenCalledWith(`npx wrangler d1 execute ${databaseName} --local --file=${projectPath}/${directory}${sqlFile} --config=${projectPath}/wrangler.toml --yes`);
  });

  it('should handle failure in SQL file execution', () => {
    mockExecSync.mockImplementation(() => {
    ...
    expect(consoleSpy).toHaveBeenCalledWith(chalk.red(`${chalk.bold('FAILED')} to execute ${sqlFile} file so skipped`));
