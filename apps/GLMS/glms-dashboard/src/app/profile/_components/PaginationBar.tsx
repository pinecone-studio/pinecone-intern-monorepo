...
export const PaginationBar = ({
  ...
  pageFunc: Dispatch<React.SetStateAction<number>>;
  pageNum: number;
  initFunc: Dispatch<React.SetStateAction<number>>;
  pageNumInit: number;
}) => {
  return (
      ...
      <button
        onClick={
          pageNum > 10
            ? () => {
                pageFunc(pageNum - 10);
                initFunc(pageNumInit - 10);
              }
            : () => {}
        }
        className="border-[1px] border-[#D6D8DB] w-[102px] h-9 rounded-lg flex gap-2 items-center justify-center text-sm font-semibold focus:opacity-30"
      >
