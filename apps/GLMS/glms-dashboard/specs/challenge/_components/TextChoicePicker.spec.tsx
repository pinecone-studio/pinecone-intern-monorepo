...
describe('ChoiceText component', () => {
  ...
  it('renders choice text correctly', () => {
    const { getByText } = render(<ChoiceText choice={mockChoice} quizId={mockQuizId} id={mockId} selectedChoice={mockSelectedChoice} handleChange={mockHandleChange} />);
    expect(getByText('Test choice')).toBeDefined();
  });

  it('applies checked style when selected choice matches', () => {
    const { container } = render(<ChoiceText choice={mockChoice} quizId={mockQuizId} id={mockId} selectedChoice={mockSelectedChoice} handleChange={mockHandleChange} />);
  ...
  it('applies unchecked style when selected choice does not match', () => {
    const { container } = render(<ChoiceText choice={mockChoice} quizId={mockQuizId} id="123456" selectedChoice={mockSelectedChoice} handleChange={mockHandleChange} />);
    const choiceDiv = container.firstChild;
  ...
  it('Render not checked checkbox if not selected', () => {
    const { getByTestId } = render(<ChoiceText choice={mockChoice} quizId={mockQuizId} id="12345" selectedChoice={mockSelectedChoice} handleChange={mockHandleChange} />);
