describe('articles page', () => {
  ...
  it('1. Should display signup form', () => {
    cy.get('[data-testid="sign-up-form-container"]').should('exist');
    cy.get('[data-testid="sign-up-form-container"]').should('be.visible');
  });
  it('2. When user enters no value on the inputs and click on the sign up button, it should display an error messages', () => {
    ...
    cy.should('contain', 'Имэйл хаягаа оруулна уу');
    cy.get('input[name="number"]').click();
    cy.get('input[name="password"]').click();
    cy.get('[data-testid="helperText"]').eq(1).should('exist');
    cy.should('contain', 'Утасны дугаараа оруулна уу');
    cy.get('input[name="password"]').click();
    cy.get('input[name="confirmPassword"]').click();
    cy.get('[data-testid="helperText"]').eq(2).should('exist');
    cy.should('contain', 'Нууц үгээ оруулна уу');
  });
  it("3. When user clicks on the signup button, it should create already signed user and shows 'Адаа гарлаа' message", () => {
    cy.get('[data-cy="Sign-Up-Button"]').should('be.disabled');
