import { MutationResolvers, ReplyStatus } from '@/graphql/generated';
import { errorTypes, graphqlErrorHandler } from '../../error';
import ReplyModel from '@/models/reply.model';

export const hideReplyByAdmin: MutationResolvers['hideReplyByAdmin'] = async (_, { hideInput }) => {
  try {
    const hiddenReply = await ReplyModel.findByIdAndUpdate(hideInput._id, { status: ReplyStatus.Hidden });
