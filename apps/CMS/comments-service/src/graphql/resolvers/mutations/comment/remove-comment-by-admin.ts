import { MutationResolvers, CommentStatus } from '@/graphql/generated';
import { errorTypes, graphqlErrorHandler } from '../../error';
import { CommentsModel } from '@/models/comment.model';

export const deleteCommentByAdmin: MutationResolvers['deleteCommentByAdmin'] = async (_, { removeInput }) => {
  try {
    const deletedComment = await CommentsModel.findByIdAndUpdate(removeInput._id, { status: CommentStatus.Deleted });
