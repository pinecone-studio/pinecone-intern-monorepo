...
const Modal = ({ isVisible, onClose }: CreateArticleModalType) => {
  ...
  return (
    <div data-testid="modalCloseBtn" id="wrapper" className=" fixed inset-2 bg-black bg-opacity-25 backdrop-blur-sm flex justify-center items-center z-11" onClick={handleCLose}>
      <div data-testid="modal" className=" w-[528px] flex flex-col bg-white relative rounded-2xl gap-6 z-12">
        <button className="py-3 text-black text-xl absolute top-[13px] right-6 font-semibold " onClick={onClose}>
          <IoClose style={{ width: 28, height: 28 }} />
        </button>
        <p className=" text-center text-2xl font-bold tracking-tight mt-12 text-[#1c2024]">
          Системээс гарахдаа итгэлтэй
          <br /> байна уу?
        </p>
