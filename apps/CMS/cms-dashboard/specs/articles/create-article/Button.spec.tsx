...
import { Button } from '../../../src/app/articles/_components/create-article/Button';

describe('Button Component', () => {
  it('1. Renders the button with label', () => {
    const { getByText } = render(<Button label="Нийтлэх" />);
    expect(getByText('Нийтлэх')).toBeDefined();
  });

  it('2. Calls onClick handler when clicked', () => {
    const handleClick = jest.fn();
    const { getByText } = render(<Button label="Нийтлэх" onClick={handleClick} />);
    fireEvent.click(getByText('Нийтлэх'));
    expect(handleClick).toHaveBeenCalledTimes(1);
  });

  it('3. Is disabled when disabled prop is true', () => {
    const { getByText } = render(<Button label="Нийтлэх" disabled />);
    const button = getByText('Нийтлэх');
    expect(button.getAttribute('disabled')).toEqual('');
  });

  it('4. Renders the button as bgColor  ', () => {
    const { getByText } = render(<Button label="Нийтлэх" bgColor="primary" />);
    const button = getByText('Нийтлэх');
  ...
  it('4. Renders the button as bgColor no p  ', () => {
    const { getByText } = render(<Button label="Нийтлэх"  bgColor="secondary"/>);
    const button = getByText('Нийтлэх');
    expect(button).not.toBeNull();
