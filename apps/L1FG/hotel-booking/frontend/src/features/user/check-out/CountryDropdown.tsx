...
const filterValidCountries = (_country: Country) => _country.emoji && _country.status !== 'deleted' && _country.ioc !== 'PRK';

const defaultOptions = countries.all.filter(filterValidCountries);

const CountryDropdownComponent = (
  { options = defaultOptions, onChange, defaultValue, disabled = false, placeholder = 'Select a country', slim = false, ...props }: CountryDropdownProps,
  ...
  useEffect(() => {
    const initialCountry = defaultValue ? options.find((country) => country.alpha3 === defaultValue) : undefined;
