...
import { getFollowings } from 'apps/L1FG/instagram/backend/src/resolvers/queries';
...
describe('Get small posts v1', () => {
  it('Should throw an authorization error', async () => {
    ...
    await expect(getFollowings({}, { input: inputAfterTrue }, { userId: '13' }, {} as GraphQLResolveInfo)).rejects.toThrow('Та нэвтэрнэ үү');
  ...
  it('Should find be called with something when after is true', async () => {
    ...
    (authenticate as jest.Mock) = mockAuthenticate;
    await getFollowings({}, { input: inputAfterTrue }, { userId: '13' }, {} as GraphQLResolveInfo);
  ...
  it('SHould find be called with something when after is false', async () => {
    ...
    (FollowerModel.find as jest.Mock) = mockFind;
    (authenticate as jest.Mock) = mockAuthenticate;
    const result = await getFollowings({}, { input: inputAfterFalse }, { userId: '13' }, {} as GraphQLResolveInfo);
  ...
  it('SHould return startCursor and endCursor empty string', async () => {
    ...
    (FollowerModel.find as jest.Mock) = mockFind;
    (authenticate as jest.Mock) = mockAuthenticate;
    const result = await getFollowings({}, { input: inputAfterTrue }, { userId: '13' }, {} as GraphQLResolveInfo);
    expect(result).toEqual({
      edges: [
        {
          cursor: 'Njc5YjFmMDEzZGViMTkwNjQyMGZjZTMx',
  ...
  it('SHould throw an error inside catch', async () => {
    ...
    (FollowerModel.find as jest.Mock) = mockFind;
    await expect(getFollowings({}, { input: inputAfterTrue }, { userId: '13' }, {} as GraphQLResolveInfo)).rejects.toThrow('Server error');
  });
});
