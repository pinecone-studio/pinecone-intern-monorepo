...
export const getNotification: QueryResolvers['getNotification'] = async (_, __, { userId }) => {
  ...
  const notifications = await NotificationModel.aggregate([
        ...
        sortOrder: {
          $switch: {
            branches: [
              ...
              { case: { $eq: ['$categoryType', 'COMMENT_LIKE'] }, then: 4 },
            ],
            default: 5,
          },
        },
    ...
    { $limit: 20 },
    {
      $group: {
        ...
        commentLike: { $push: { $cond: [{ $eq: ['$categoryType', 'COMMENT_LIKE'] }, '$$ROOT', '$$REMOVE'] } },
      },
    },
    {
      $match: {
        $or: [{ postLike: { $ne: [] } }, { comment: { $ne: [] } }, { request: { $ne: [] } }, { commentLike: { $ne: [] } }],
      },
    },
  ]);
  ...
  return notifications.reduce((acc, group) => {
    acc[group._id] = {
      ...
      commentLike: group.commentLike.length > 0 ? group.commentLike : null,
    };
    return acc;
  }, {});
};
