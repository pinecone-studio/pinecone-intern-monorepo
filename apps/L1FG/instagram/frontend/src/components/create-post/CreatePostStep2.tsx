...
export const CreatePostStep2: React.FC<CreatePostStep2Props> = ({ step, setStep, images, setOpenCreatePostModal, loading }) => {
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" data-testid="create-post-step2-modal">
        <div className="bg-white rounded-xl max-w-xl w-full overflow-hidden">
          ...
          <div className="relative h-[626px]" data-testid="image-container">
            ...
            <Image src={images[0]} alt="Uploaded image" width={800} height={600} className="object-cover rounded-b-lg" data-testid="selected-image" />
          </div>
        </div>
      </div>

      {showFinalStep && <CreatePost images={images} setStep={setStep} />}
    </>
  );
};
