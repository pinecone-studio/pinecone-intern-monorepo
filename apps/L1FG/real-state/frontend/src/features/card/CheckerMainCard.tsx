...
export const MainCard = ({ value }: { value: Post }) => {
  return (
    <Link className="flex items-center w-full justify-center" href={`/estates/${value._id}`}>
      <div className="w-full h-[300px] shadow-lg border rounded-lg flex flex-col">
        <Image src={`${value.propertyDetail.images[0]}`} alt={'townName'} className=" rounded-md object-center overflow-hidden ]" height={150} width={300} />
