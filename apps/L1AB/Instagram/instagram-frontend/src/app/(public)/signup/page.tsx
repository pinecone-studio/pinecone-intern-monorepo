...
const validationSchema = Yup.object({
  mobileNumberorEmail: Yup.string().email('Invalid email or mobile number').required('Mobile number or email is required'),
  ...
  username: Yup.string().min(3, 'Username must be at least 3 characters').required('Username must be at least 3 characters'),
});
...
const Page = () => {
  ...
  const handleSubmit = async (values: { mobileNumberorEmail: string; password: string; fullname: string; username: string }) => {
    const { data } = await signup({
      variables: {
        fullname: values.fullname,
        username: values.username,
        email: values.mobileNumberorEmail,
  ...
  return (
    <div className="min-h-screen flex flex-col items-center py-20 bg-gray-100 gap-4" data-cy="Sign-Up-Page">
      <div className=" w-[364px] flex flex-col items-center rounded-xl py-8 gap-4 bg-white">
        <InstagramIcon />
        <p className="w-[70%] flex justify-center text-center">Sign up to see photos and videos from your friends</p>
        <Formik initialValues={{ mobileNumberorEmail: '', password: '', fullname: '', username: '' }} validationSchema={validationSchema} onSubmit={handleSubmit}>
          {({ isSubmitting, errors }) => (
            <Form className="w-[84%]">
              <div className="mb-3">
                <label htmlFor="mobileNumberorEmail" className="sr-only">
              ...
              <div className="text-gray-400 text-sm pt-2">
                <p>
                  People who use our service may have uploaded your contact information to Instagram. <button className="text-blue-500">Learn</button>
                  <button className="text-blue-500 px-32">More</button>
                </p>
              </div>
