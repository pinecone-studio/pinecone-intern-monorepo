  ...
  return (
    <>
      <motion.nav
        ...
        className={`z-10 drawer bg-white fixed space-y-5  border h-full left-[80px] rounded-r-2xl divide-y divide-slate-200 transition-transform duration-100 ${isOpen ? 'w-[411px]' : 'w-0'}`}
        style={{
        ...
        <div>
          ...
          {!searchInput
            ? visitedUsers.map((user, i) => (
                <div data-testid={`search-visited-${i}`} key={i} className="h-[58px] w-full bg-white flex items-center justify-between px-6 hover:bg-gray-100">
