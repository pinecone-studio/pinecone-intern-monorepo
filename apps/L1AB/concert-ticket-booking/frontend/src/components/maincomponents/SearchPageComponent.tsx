...
export const SearchPageComponent = () => {
  ...
  if (loading) {
    return (
      <div>
        <div className="flex gap-2 px-28 py-12 max-sm:px-3">
  ...
  return (
    <div className="text-white flex flex-col py-12" data-testid="searchpagecomponent" data-cy="getEvents">
      <div className="flex gap-2 items-center px-28 max-sm:px-3">
      ...
      <div className="grid grid-cols-3 gap-8 py-24 px-28 max-sm:grid  max-sm:grid-cols-1 max-sm:px-3 max-sm:py-4">
        {searchedData?.length ? searchedData.map((event) => <EventCard key={event._id} {...event} />) : <p className="text-[#A1A1AA] text-2xl col-span-3 text-center max-sm:text-sm">Тохирох үйл явдал олдсонгүй.</p>}
      </div>
    </div>
  );
};
