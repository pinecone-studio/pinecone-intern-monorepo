...
export const StageStyle = ({ venue }: StageStyleProps) => {
  ...
  return (
    <>
      {demoEvent ? (
        <>
          <div className="h-[600px] w-[650px] relative max-xl:h-[540px] max-xl:w-[540px]   max-md:block max-md:h-[340px] max-md:w-[540px] max-sm:block max-sm:h-[340px] max-sm:w-full">
          ...
          <>
            ...
            <div className="h-[684px] w-[723px] max-sm:hidden max-md:hidden mt-40">
              <div className="relative mx-32 my-48">
                ...
                {sections.map(({ component: Component, position, text }, index) => (
                  <div key={index} className={`absolute ${position} group`}>
                    <Component />
                    <div className="absolute top-10 left-0 w-max bg-black text-white text-sm py-1 px-2 rounded opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-50 ">{text}</div>
                  </div>
                ))}
              </div>
            </div>
