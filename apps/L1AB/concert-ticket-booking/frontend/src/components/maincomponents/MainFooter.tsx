...
export const MainFooter = () => {
  ...
  return (
    <div className="border-t border-[#27272A] py-24 px-28 flex justify-between items-center">
      <div className="h-fit flex flex-col gap-3">
        <div className="flex items-center ">
          <GoDotFill className="w-8 h-8 text-[#00B7F4]" />
          <h1 className="text-white font-semibold text-xl">TICKET BOOKING</h1>
        </div>
        <p className="text-[#818181] text-sm font-normal ml-2">© 2024 Booking Mongolia. All Rights Reserved.</p>
      </div>
      <div className="flex flex-col gap-3 ">
        <p className="text-[#818181] text-sm font-normal ">Contact Information</p>
        <div className="flex gap-12">
          {AddressMock.map((item, index) => (
            <div key={index} className="flex items-center text-white gap-4">
              <item.icon />
              <div>
                <p className="font-light text-sm text-[#818181]">{item.name}</p>
