...
export const DeletedEvent = ({ eventId }: { eventId: string }) => {
  ...
  const handleUpdateStatus = async () => {
    const status = selectedOption === 'Тийм' ? 'Deleted' : 'Regular';
    ...
    <div data-testid="AdvertisedEvent-Component">
      <AlertDialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <AlertDialogTrigger asChild>
          <TrashIcon data-testid="AdvertisedEvent-TriggerButton" className="h-5 w-5 bg-[#F4F4F5] rounded cursor-pointer p-[2px]" />
        </AlertDialogTrigger>
          ...
          <Card className="w-full border-none gap-4" data-testid="AdvertisedEvent-Card">
            <CardHeader className="gap-4" data-testid="AdvertisedEvent-CardHeader">
              <div className="flex justify-between items-center" data-testid="AdvertisedEvent-HeaderActions">
                <CardTitle data-testid="AdvertisedEvent-Title">Тоглолтыг устгах</CardTitle>
