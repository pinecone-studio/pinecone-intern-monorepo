...
export const updateStudent: MutationResolvers['updateStudent'] = async (_, { updateInput }) => {
  const { _id, firstName, lastName, studentCode, profileImgUrl, classId } = updateInput;
  try {
    const updateStudent = await StudentsModel.findByIdAndUpdate(_id, { firstName, lastName, studentCode, profileImgUrl, classId });
