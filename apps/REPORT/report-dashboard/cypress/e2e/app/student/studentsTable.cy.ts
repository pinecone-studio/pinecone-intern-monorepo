...
describe('Students Table', () => {
  ...
  it('displays students data correctly', () => {
    ...
    studentsData.forEach((student, index) => {
      ...
      // Check the active state representation (assuming it's 'Active' or 'Inactive')
      const expectedActiveText = student.active ? 'Active' : 'Inactive';
      cy.get('tbody').find(`[data-test="chip"]:eq(${index})`).should('contain.active', expectedActiveText);
    });
  });
