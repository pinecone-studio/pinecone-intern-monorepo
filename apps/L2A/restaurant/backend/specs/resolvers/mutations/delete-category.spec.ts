...
describe("deleteCategory", () => {
  ...
  it("should throw an error if _id is missing", async () => {
    await expect(deleteCategory(null, { _id: "" })).rejects.toThrow(
      /Category ID is required/
    );
  });
  it("should throw an error if category not found", async () => {
    (categoryModel.findByIdAndDelete as jest.Mock).mockResolvedValue(null);
    await expect(deleteCategory(null, { _id: "not-found" })).rejects.toThrow(
      /Category not found/
    );
  });
  it("should throw an error if deletion fails", async () => {
    ...
    await expect(deleteCategory(null, { _id: "123" })).rejects.toThrow(
