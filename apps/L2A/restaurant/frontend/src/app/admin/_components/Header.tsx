  ...
  return (
    <header className="w-full shadow-sm" data-testid="admin-header">
      <div className="container mx-auto px-6 py-4 flex items-center justify-between">
        <nav className="flex gap-6 text-sm font-medium" data-testid="nav-bar">
          {menuItems.map((item) => (
            <Link href={item.link} key={item.name} data-testid={`nav-item-${item.name}`}>
              <p data-testid={"Food"} className="text-lg cursor-pointer px-2 py-1 rounded hover:bg-gray-100">{item.name}</p>
            </Link>
          ))}
        </nav>
