...
import { completeSignup } from 'apps/L2A/real-estate/real-estate-backend/src/resolvers/mutations/user/complete-user';
...
describe('completeSignup resolver', () => {
  const mockArgs = {
  ...
  it('should complete signup successfully', async () => {
    ...
    const result = await completeSignup({}, mockArgs);
    ...
    expect(USER_MODEL.findOneAndUpdate).toHaveBeenCalledWith(
  ...
  it('should throw error when user not found', async () => {
    ...
    await expect(completeSignup({}, mockArgs))
      .rejects
  ...
  it('should throw error when password already set', async () => {
    ...
    (USER_MODEL.findOne as jest.Mock).mockResolvedValueOnce(userWithPassword);

    await expect(completeSignup({}, mockArgs))
      .rejects
      .toThrow('Password already set — try logging in');
