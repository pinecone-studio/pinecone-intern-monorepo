...
import { requestOTP } from 'apps/L2A/real-estate/real-estate-backend/src/resolvers/mutations/otp/request-otp';
...
jest.mock('apps/L2A/real-estate/real-estate-backend/src/resolvers/mutations/otp/send-email', () => ({
  sendEmail: jest.fn(),
}));
...
describe('requestOTP', () => {
  ...
  it('should generate OTP, update DB, send email, and return true', async () => {
    ...
    expect(OTP.findOneAndUpdate).toHaveBeenCalledWith({ email: mockEmail.toLowerCase().trim() }, expect.objectContaining({ otp: mockOTP }), { upsert: true, new: true });
