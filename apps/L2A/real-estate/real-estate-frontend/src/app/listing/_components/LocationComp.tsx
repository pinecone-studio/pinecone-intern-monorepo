    ...
    return (
        <>
            <div>
                ...
                <select onChange={(e) => setCity(e.target.value)} defaultValue="" className="w-full mb-2 border rounded px-2 py-1 text-sm focus:outline-none focus:border-black focus:ring-black" data-testid="select-city" data-cy="select-city">
                    <option value="">Бүх хотууд</option>
                    {uniqueCity?.map((item, index) => <option key={index}>{item}</option>)}
                </select>
                <select onChange={(e) => setDistrict(e.target.value)} defaultValue="" className="w-full border rounded px-2 py-1 text-sm focus:outline-none focus:border-black focus:ring-black" data-testid="select-district" data-cy="select-district">
