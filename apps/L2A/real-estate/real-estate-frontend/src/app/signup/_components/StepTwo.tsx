    ...
    return (
        <>
            <div className="flex flex-col items-center justify-center">
                ...
                {!resendEnabled && <p className="text-[#09090B] mt-4" data-cy="resend-otp-button">Send again({timer})</p>}
                {resendEnabled && <button className="text-[15px] font-500 text-[#09090B] mt-4" onClick={sendOTP}>Send again({timer})</button>}
                {loading && <p className="text-gray-500 mt-2" data-cy="otp-loading">Verifying OTP...</p>}
            </div>
        </>
    )
}