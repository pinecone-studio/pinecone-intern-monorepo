...
const EditEventInfo = ({ concert, idx }: Props) => {
  ...
  const onSubmit = async (values: z.infer<typeof EditEventInfoSchema>) => {
    await UpdateEventInfo({ variables: { ...values, concertId: concert.id }, refetchQueries: ['concerts'] });
  };
  return (
    <Dialog>
      ...
      <Snackbar autoHideDuration={500} anchorOrigin={{ vertical: 'top', horizontal: 'center' }} open={!!data} message="Мэдээлэл амжилттай өөрчлөгдлөө!" />
