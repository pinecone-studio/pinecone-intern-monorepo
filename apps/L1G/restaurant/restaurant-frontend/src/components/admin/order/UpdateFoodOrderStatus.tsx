...
const createFormSchema = (previousStatus: string) =>
  z.object({
    status: z.string({ required_error: 'Төлөв өө сонгоно уу!' }).refine((val) => val !== previousStatus, {
      message: 'Шинэ төлөв хуучинтай ижил байна!',
    }),
  });
...
export const UpdateFoodOrderStatus = ({ orderId, orderStatus, refetch }: { orderId: string; orderStatus: string; refetch: () => Promise<ApolloQueryResult<GetFoodOrdersQuery>> }) => {
  ...
  const form = useForm<FormSchema>({
    resolver: zodResolver(createFormSchema(orderStatus)),
    defaultValues: {
      status: orderStatus,
    },
  });

  const handleUpdateStatus = async (values: FormSchema) => {
    await updateFoodOrderStatus({
      variables: {
        input: {
          orderId,
          status: values.status as FoodOrderStatus,
        },
      },
    });
          ...
          render={({ field }) => (
            <FormItem>
              <FormControl>
                <select
                  {...field}
                  className="h-[36px] w-fit min-w-[120px] rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50"
