...
export const DiscountUpdateDialog = ({ refetch, discountId, discountName, discountRate, startDate, endDate }: DiscountUpdateProps) => {
  ...
  const form = useForm<z.infer<typeof formSchemaDiscount>>({
    ...
    defaultValues: {
      ...
      discountDate: {
        from: new Date(parseInt(startDate)) || undefined,
        to: new Date(parseInt(endDate)) || undefined,
      },
    },
  });
                ...
                <FormItem className="flex flex-col">
                  <Popover open={open} onOpenChange={setOpen}>
                        ...
                        >
                          ...
                          {startDate ? (
                            endDate ? (
                              ...
                              <>{format(new Date(parseInt(startDate)), 'LLL dd, y')}</>
                            )
                          ) : (
                    ...
                    <PopoverContent className="w-auto p-0 flex" align="start">
                      <Calendar
                        data-testid="discount-update-calendar"
                        initialFocus
                        mode="range"
                        numberOfMonths={2}
