...
import { Category, GetCategoriesQuery, useDeleteFoodFromCategoryMutation } from '@/generated';
...
export const CategoryFoodsCard = ({ activeMenu, data, refetch }: { activeMenu: string; data: Category[]; refetch: () => Promise<ApolloQueryResult<GetCategoriesQuery>> }) => {
  const [deleteCategoryFood] = useDeleteFoodFromCategoryMutation();

  const handleDeleteCategoryFood = async (categoryId: string, foodId: string) => {
    try {
      await deleteCategoryFood({
        variables: {
      ...
      await refetch();
      toast.success('Цэснээс амжилттай хасагдлаа!', {
        position: 'bottom-right',
      });
    } catch (error) {
      toast.error('Цэснээс хасахад алдаа гарлаа!', {
        position: 'bottom-right',
      });
    }
  };
        ...
        (category) =>
          ...
          category?.food?.map((food) => (
            <div key={food?.foodId} className="flex flex-col gap-4">
              <div data-testid={`food-with-category-${food?.foodId}`} className="flex w-full h-[87px] justify-between p-0">
                ...
                <div className="flex flex-col gap-2 w-[345px] h-[87px]">
                  <p data-testid="to-category-foodName" className="text-base font-light leading-[20px] text-[#09090B]">
                    {food?.foodName}
                  </p>
                ...
                <DeleteDialog
                  ...
                  submitText="Хасах"
                  onClick={() => handleDeleteCategoryFood(category.categoryId, food?.foodId as string)}
                >
                  <div data-testid={`from-category-delete-dialog-${food?.foodId}`}>
