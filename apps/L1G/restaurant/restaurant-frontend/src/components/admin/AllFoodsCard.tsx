...
import { FoodUpdateDialog } from './FoodUpdateDialog';
...
export const AllFoodsCard = () => {
  ...
  const handleDeleteButton = async (foodId: string) => {
    try {
      ...
      if (error.message.includes('Food with ID')) {
        toast.error(`${foodId} кодтой хоол олдсонгүй!`, {
          position: 'bottom-right',
        });
      }
  ...
  return (
    <div className="flex flex-col w-[600px] h-[691px] gap-5 top-[219px] left-[500px]">
      ...
      {loading ? (
        <Card data-testid="allfoods-loading" className="flex flex-col w-full h-[631px] p-8 gap-4 rounded-lg border solid border-[#E4E4E7] bg-[#FFFFFF] text-start text-base">
          Loading...
        </Card>
      ) : error ? (
            ...
            <div key={food?.foodId} className="flex flex-col gap-4">
              <CardContent className="flex w-full h-[87px] justify-between p-0">
                ...
                <DeleteDialog title="Устгах" comment="Устгахдаа итгэлтэй байна уу" submitText="Устгах" onClick={() => handleDeleteButton(food?.foodId as string)}>
                  <div data-testid={`delete-dialog-trigger-${food?.foodId}`}>
