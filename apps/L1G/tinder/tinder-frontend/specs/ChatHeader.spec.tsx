...
jest.mock('@/components/UnmatchButton', () => {
  return { __esModule: true, default: () => <button data-testid="unmatch-button">Unmatch</button> };
});
jest.mock('@/components/ViewProfile', () => {
  return { __esModule: true, default: (_props: { user: ChatUser }) => <button data-testid="view-profile-button">View Profile</button> };
});
...
describe('ChatHeader', () => {
  ...
  it('renders UnmatchButton component when matchId is provided', () => {
    render(<ChatHeader user={mockUser} matchId={matchId} />);
    const unmatchButton = screen.getByTestId('unmatch-button');
    expect(unmatchButton).toBeInTheDocument();
  });
  it('does NOT render UnmatchButton component when matchId is NOT provided', () => {
    render(<ChatHeader user={mockUser} matchId={undefined} />);
    const unmatchButton = screen.queryByTestId('unmatch-button');
    expect(unmatchButton).not.toBeInTheDocument();
  });
  ...
  it('handles user with different data correctly', () => {
    ...
    render(<ChatHeader user={differentUser} matchId={matchId} />);
  ...
  it('handles user with long profession name', () => {
    ...
    render(<ChatHeader user={userWithLongProfession} matchId={matchId} />);
  ...
  it('handles user with edge case ages', () => {
    ...
    render(<ChatHeader user={youngUser} matchId={matchId} />);
  ...
  describe('component integration', () => {
    it('passes correct user data to child components', () => {
      const customUser: ChatUser = { id: '5', name: 'Emma', age: 26, profession: 'Marketing Manager', images: ['emma.jpg'], dateOfBirth: '1997-12-05', startedConversation: true };

      render(<ChatHeader user={customUser} matchId={matchId} />);
      const avatar = screen.getByTestId('avatar');
