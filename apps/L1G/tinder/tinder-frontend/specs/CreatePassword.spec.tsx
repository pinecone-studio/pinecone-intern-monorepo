...
describe('CreatePassword Component', () => {
  ...
  it('shows server error message when signup throws exception', async () => {
    const testError = new Error('Server exploded!');
    mockSignup.mockRejectedValue(testError);
  ...
  // NEW TEST: This should cover the missing branch on line 65
  // Testing the fallback error message when caught exception has no message
  it('shows default error message when signup throws exception without message', async () => {
    // Create an error object without a message property
    const errorWithoutMessage = new Error();
    delete errorWithoutMessage.message; // Remove message property

    mockSignup.mockRejectedValue(errorWithoutMessage);
    ...
    // This should test the fallback: e?.message || 'Something went wrong.'
    expect(await screen.findByText('Something went wrong.')).toBeInTheDocument();
  });

  // ADDITIONAL TEST: Testing with null error object
  it('shows default error message when signup throws null', async () => {
    mockSignup.mockRejectedValue(null);
  ...
  // ADDITIONAL TEST: Testing with undefined error object
  it('shows default error message when signup throws undefined', async () => {
