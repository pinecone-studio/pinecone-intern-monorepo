...
export const MyProfile = () => {
  ...
  return (
    <div data-testid="my-profile" className="w-full h-screen flex flex-col gap-6">
      <MyProfileHeader isOpen={isOpen} setIsOpen={setIsOpen} />
      <div className="flex flex-1 flex-col md:flex-row justify-start items-start gap-6 lg:gap-12 px-4 sm:px-6 md:px-10 pb-6 md:overflow-hidden">
        <SidebarMenu menu={menu} setMenu={setMenu} isOpen={isOpen} setIsOpen={setIsOpen} />
        <div data-testid="menu-content" className="w-full flex-1 overflow-y-auto border-2 border-red-500 " style={{ maxHeight: 'calc(100vh - 4rem)' }}>
    ...
    <>
      ...
      <div className="fixed inset-0 z-50 md:hidden" aria-hidden={!isOpen}>
        ...
        <div
          ...
          className={clsx(
            'fixed top-0 left-0 w-[280px] h-screen max-h-[90vh] overflow-y-auto bg-white shadow-xl z-50 flex flex-col p-6 transform transition-transform duration-300',
