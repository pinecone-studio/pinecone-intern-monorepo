            ...
            <div className="flex flex-col gap-3">
              {items.map(({ label, value }) => (
                <Button
                  ...
                  className={`rounded-lg py-3 px-4 text-base font-sans font-medium text-[#09090B] justify-start transition-colors',
                    ${menu === value ? 'bg-[#F4F4F5] hover:bg-[#E4E4E7]' : 'bg-transparent hover:bg-[#F4F4F5]'}`}
                >
                  {label}
                </Button>
...
export const MyProfileHeader = ({ isOpen, setIsOpen }: { isOpen: boolean; setIsOpen: (_open: boolean) => void }) => {
  return (
