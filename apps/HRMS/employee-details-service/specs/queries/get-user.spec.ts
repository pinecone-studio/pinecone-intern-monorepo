...
import { getHrmsUser } from '@/graphql/resolvers/queries';
...
describe('getHrmsUser', () => {
  it('should get a user', async () => {
    const result = await getHrmsUser!({}, { _id: '1' }, {}, {} as GraphQLResolveInfo);
  ...
  it('should throw an error if the user cannot be found', async () => {
    try {
      await getHrmsUser!({}, { _id: '2' }, {}, {} as GraphQLResolveInfo);
    } catch (error) {
  ...
  it('should throw an error if an error occurs during user retrieval', async () => {
    try {
      await getHrmsUser!({}, { _id: '1' }, {}, {} as GraphQLResolveInfo);
    } catch (error) {
