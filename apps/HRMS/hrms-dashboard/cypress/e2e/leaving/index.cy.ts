describe('Leaving page', () => {
  ...
  // //   cy.get('[data-cy="open-request"]').contains('Чөлөөний хуудас бөглөх').click();
  // //   cy.get('[data-cy="request-modal"]').contains('Чөлөөний хуудас бөглөх');
  // //   cy.get('input[name="step1UserName"]').invoke('val').should('have.value', '');
  // //   cy.get('input[name="step1LeaveType"]').invoke('val').should('have.value', '');
  ...
  it('12. should test if ternary function works and displays label and field properly', () => {
    cy.get('[data-cy="open-request"]').contains('Чөлөөний хуудас бөглөх').click();
    cy.get('[data-cy="request-modal"]').contains('Чөлөөний хуудас бөглөх');

    cy.get('[data-cy="step1Label"]').then(($label) => {
      if ($label.text().includes('Огноо')) {
        cy.get('input').eq(0).should('have.attr', 'name', 'step1Date').should('be.visible');
      }
      // if ($label.text().includes('Нэрээ сонгоно уу')) {
      //   cy.get('input[name="step1UserName"]').should('be.visible');
      // }
      // if ($label.text().includes('Шалтгаанаа сонгоно уу')) {
      //   cy.get('input[name="step1LeaveType"]').should('be.visible');
      // }
    });
  });
});
