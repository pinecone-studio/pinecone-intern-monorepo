...
export const PersonalInformation = (props: Employee) => {
  ...
  const { lastName, jobTitle, email } = props;
  return (
    <Stack sx={{ maxWidth: '358px', borderRadius: '20px', display: 'flex', flexDirection: 'column', gap: '40px' }}>
      ...
      <Typography sx={{ fontSize: '18px', color: '#000000', display: 'flex', justifyContent: 'center' }} fontWeight={525}>
        {lastName}
      </Typography>
      <Stack sx={{ display: 'flex', justifyContent: 'center', flexDirection: 'column', gap: '16px', paddingLeft: '60px', fontWeight: 'light' }}>
        ...
        <Box sx={boxStyle}>
          ...
          <Typography>+97680556021</Typography>
        </Box>
        <Box sx={boxStyle}>
        ...
        <Box sx={boxStyle}>
          <LocationOn />
          <Typography>Ulaanbaatar ,Mongolia</Typography>
        </Box>
      </Stack>
    </Stack>
  );
