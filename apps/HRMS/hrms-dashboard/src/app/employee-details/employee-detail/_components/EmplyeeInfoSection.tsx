    ...
    <div className="h-full flex flex-col gap-3">
      ...
      {infoFields.map((field, index) => (
        <div key={index} className="mb-2 flex flex-col">
          <p data-testid={`field-title-${index}`} className="border-none font-normal text-base p-0">
            {field.title}
          </p>
          <p data-testid="status-value" className="border-none font-semibold text-base p-0 text-black">
            {field.value}
          </p>
        </div>
      ))}
