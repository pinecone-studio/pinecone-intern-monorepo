...
const RequestDetails = ({ data }: { data: Details }) => {
  ...
  const endDate = data.endHour ? new Date(data.endHour).toLocaleDateString('en-US', { month: 'numeric', day: 'numeric' }) : ''
  ...
  return (
    <div className="flex flex-col gap-3 w-[565px] h-[312px] bg-white rounded-lg p-5" data-testid="request-details">
      ...
      <p className="font-semibold text-black">{data.durationType}</p>
