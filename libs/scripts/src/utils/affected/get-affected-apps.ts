...
export const getChangedFiles = (): string => {
  ...
  const command = process.env.base
    ? `${nxBaseCommand} ${`${nxBase}...${nxHead}`}`
    : baseCommand;

  return execAndTrim(command);
};

export const getAffectedProjectsList = ({
  additionalParams = '',
}: { additionalParams?: string } = {}): string[] => {
  const { nxBase, nxHead } = getNxHeadAndBase({ tag: true });
  const baseCommand = `bunx nx show projects --affected ${additionalParams}`;
  const command = process.env.base
    ? `${baseCommand} ${nxBase} ${nxHead}`
    : baseCommand;
  return execAndTrim(command)
    .split('\n')
    .filter((app) => app.trim() !== '');
...
export const getProjectPath = (affectedProject: string): string => {
  if (affectedProjectPaths[affectedProject])
    return affectedProjectPaths[affectedProject];

  const command = `bunx nx show project ${affectedProject}`;
...
export const checkMatchingRoot = (
  directories: string[],
  filePath: string
): string | undefined => {
  const matchingDirectory = directories.find((directory) =>
    filePath.includes(getProjectPath(directory))
  );
  return matchingDirectory;
};

export const findMatchingDirectory = (
  logLine: string,
  directories: string[]
): string | undefined => {
