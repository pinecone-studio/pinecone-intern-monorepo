...
export const getProjectInfo = (project: string) => {
  const command = `yarn nx show project ${project}`;
  const projectInfo = execSync(command).toString().trim();
  console.log('projectnfoddddp', projectInfo);
...
export const doesProjectHaveE2ECommand = (project: string) => {
  ...
  return projectInfo.targets['cypress:run'];
};
...
export const runE2ECommand = async (projectWithE2E: string | undefined, project: ProjectPreview) => {
  if (projectWithE2E) {
    const command = `yarn nx cypress:run ${project.name} --verbose --baseUrl=${project.url} --skipServe --devServerTarget=""`;
